...
export function SegmentGroupedBarChart({ 
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      // Format value based on size - use formatNumber for better readability
      const formattedValue = formatNumber(hoveredItem.value)
      ...
      return (
        <div className={`p-4 rounded-lg border-2 shadow-lg ${
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-base">{formattedValue}</span>
